    ...
    NumMatrix(vector<vector<int>>& matrix) {
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < columns; j++){
                ...
                int top = i > 0 ? prefixSum[i - 1][j] : 0;          //get the top
                int left = j > 0 ? prefixSum[i][j - 1] : 0;         //get the left;
                int topLeft = (i > 0 && j > 0) ? prefixSum[i - 1][j -1] : 0;   //get top left
 ...
 */
 ...
 int main(){
    ...
    NumMatrix test(input);
    
    cout<<test.sumRegion(0, 2, 3, 4);
 }